<x-app-layout>
    <x-slot:title>Editar Manutenção</x-slot:title>
    ...
    <div class="mb-8">
        ...
        <h1 class="text-2xl font-bold text-white">Editar Manutenção</h1>
    </div>

    <div class="max-w-2xl">
        <form method="POST" action="{{ route('veiculos.manutencoes.update', [$veiculo, $manutencao]) }}" class="space-y-5">
            @csrf @method('PUT')
            ...
            <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-5">
                ...
                <div>
                    ...
                    <select name="tipo" required
                        ...
                        @foreach($tipos as $val => $label)
                            <option value="{{ $val }}" {{ old('tipo', $manutencao->tipo) === $val ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="descricao" value="{{ old('descricao', $manutencao->descricao) }}" required
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="data" value="{{ old('data', $manutencao->data->format('Y-m-d')) }}" required
                            class="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-emerald-500 text-sm">
                    </div>
                    <div>
                        <label class="block text-sm text-slate-300 mb-1.5">KM no momento</label>
                        <input type="number" name="km_na_manutencao" value="{{ old('km_na_manutencao', $manutencao->km_na_manutencao) }}" min="0"
                            class="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 text-sm">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="valor" value="{{ old('valor', $manutencao->valor_centavos ? $manutencao->valor_centavos / 100 : null) }}" step="0.01" min="0"
                            class="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 text-sm">
                    </div>
                    <div>
                        <label class="block text-sm text-slate-300 mb-1.5">Oficina</label>
                        <input type="text" name="oficina" value="{{ old('oficina', $manutencao->oficina) }}"
                            class="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 text-sm">
                    </div>
                </div>

                <div class="border-t border-slate-800 pt-4">
                    <p class="text-xs text-slate-500 uppercase tracking-wide mb-3">Alerta para próxima manutenção</p>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm text-slate-300 mb-1.5">Próxima aos KM</label>
                            <input type="number" name="proxima_km" value="{{ old('proxima_km', $manutencao->proxima_km) }}" min="0"
                                class="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 text-sm">
                        </div>
                        <div>
                            <label class="block text-sm text-slate-300 mb-1.5">Próxima em data</label>
                            <input type="date" name="proxima_data" value="{{ old('proxima_data', $manutencao->proxima_data?->format('Y-m-d')) }}"
                                class="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-emerald-500 text-sm">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select name="tipo_uso"
                        class="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-emerald-500 text-sm">
                        <option value="pessoal" {{ old('tipo_uso', $manutencao->tipo_uso) === 'pessoal' ? 'selected' : '' }}>Pessoal</option>
                        <option value="empresa" {{ old('tipo_uso', $manutencao->tipo_uso) === 'empresa' ? 'selected' : '' }}>Empresa</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <textarea name="observacao" rows="2"
                        class="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 text-sm resize-none">{{ old('observacao', $manutencao->observacao) }}</textarea>
                </div>
            </div>
