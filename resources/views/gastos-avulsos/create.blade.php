<x-app-layout>
    <x-slot name="title">Novo Gasto Avulso</x-slot>
    ...
    <div class="max-w-2xl">
        <div class="flex items-center gap-3 mb-8">
            ...
            <h1 class="text-2xl font-bold text-white">Novo Gasto Avulso</h1>
        </div>

        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            <form method="POST" action="{{ route('gastos-avulsos.store') }}" class="space-y-5">
                @csrf

                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-1.5">Descricao *</label>
                    <input type="text" name="descricao" value="{{ old('descricao') }}" placeholder="Ex: Consulta medica, Material escritorio..."
