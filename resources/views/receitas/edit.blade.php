<x-app-layout>
    ...
    <div class="max-w-2xl">
        <div class="flex items-center gap-3 mb-8">
            ...
            <h1 class="text-2xl font-bold text-white">Editar Entrada</h1>
        </div>

        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            <form method="POST" action="{{ route('receitas.update', $receita) }}" class="space-y-5"
                x-data="{ recorrente: {{ old('recorrente', $receita->recorrente) ? 'true' : 'false' }} }">
                @csrf @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="descricao" value="{{ old('descricao', $receita->descricao) }}"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" step="0.01" name="valor"
                            value="{{ old('valor', number_format($receita->valor_centavos / 100, 2, '.', '')) }}"
                            class="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-emerald-500" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-300 mb-1.5">Data *</label>
                        <input type="date" name="data" value="{{ old('data', $receita->data->format('Y-m-d')) }}"
                            class="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-emerald-500" required>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex gap-3">
                        <label class="flex-1 flex items-center gap-3 p-3 bg-slate-800 border border-slate-700 rounded-xl cursor-pointer">
                            <input type="radio" name="tipo" value="pessoal" {{ old('tipo', $receita->tipo) === 'pessoal' ? 'checked' : '' }} class="text-emerald-500">
                            <span class="text-sm text-white">Pessoal</span>
                        </label>
                        <label class="flex-1 flex items-center gap-3 p-3 bg-slate-800 border border-slate-700 rounded-xl cursor-pointer">
                            <input type="radio" name="tipo" value="empresa" {{ old('tipo', $receita->tipo) === 'empresa' ? 'checked' : '' }} class="text-blue-500">
                ...
                <div>
                    ...
                    <textarea name="observacao" rows="2" class="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-emerald-500 resize-none">{{ old('observacao', $receita->observacao) }}</textarea>
                </div>
