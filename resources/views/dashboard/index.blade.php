<x-app-layout>
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        ...
        <a href="{{ route('dashboard.print', array_filter(['mes' => $mes, 'tipo' => $tipo !== 'todos' ? $tipo : null, 'categoria_id' => $categoriaId])) }}"
           target="_blank"
           class="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 hover:text-white text-sm font-medium rounded-xl transition-colors whitespace-nowrap">
    ...
    @if($categoriaId)
        {{-- Modo filtrado por categoria: mostra só o resumo da categoria e os gastos avulsos --}}
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
            <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex items-center gap-4">
                @if($catAtiva?->cor)
                    <span class="w-4 h-4 rounded-full flex-shrink-0" style="background-color: {{ $catAtiva->cor }}"></span>
                @endif
                <div>
                    <p class="text-slate-400 text-xs font-medium uppercase tracking-wide mb-1">Categoria</p>
                    <p class="text-xl font-bold text-white">{{ $catAtiva?->nome ?? 'Categoria' }}</p>
                </div>
            </div>
            <div class="bg-slate-900 border border-slate-800 rounded-2xl p-5">
                <p class="text-slate-400 text-xs font-medium uppercase tracking-wide mb-2">Total no mês</p>
                <p class="text-3xl font-bold text-white">R$ {{ number_format($totalAvulsosMes / 100, 2, ',', '.') }}</p>
                <p class="text-slate-500 text-xs mt-1">{{ $gastosAvulsosMes->count() }} {{ $gastosAvulsosMes->count() === 1 ? 'gasto' : 'gastos' }}</p>
            </div>
        </div>
        ...
        <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6">
            <div class="flex items-center justify-between mb-4">
                <h2 class="font-bold text-white flex items-center gap-2">
                    @if($catAtiva?->cor)
                        <span class="w-3 h-3 rounded-full" style="background-color: {{ $catAtiva->cor }}"></span>
                    @endif
                    {{ $catAtiva?->nome ?? 'Gastos' }}
                </h2>
                <a href="{{ route('gastos-avulsos.index') }}" class="text-xs text-emerald-400 hover:text-emerald-300">Ver todos</a>
            </div>
            @if($gastosAvulsosMes->isEmpty())
                <p class="text-slate-500 text-sm py-8 text-center">Nenhum gasto nesta categoria em {{ \Carbon\Carbon::createFromFormat('Y-m', $mes)->translatedFormat('F/Y') }}.</p>
            @else
                <div class="space-y-1">
                    @foreach($gastosAvulsosMes as $item)
                        <div class="flex items-center justify-between py-2.5 border-b border-slate-800/50 last:border-0 last:pb-0">
                            <div>
                                ...
                                <p class="text-xs text-slate-400">{{ $item->data->format('d/m/Y') }}</p>
                            </div>
                            <span class="text-sm font-semibold text-slate-300 ml-4 whitespace-nowrap">R$ {{ number_format($item->valor_centavos / 100, 2, ',', '.') }}</span>
                        </div>
                    @endforeach
                </div>
